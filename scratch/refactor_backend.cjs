...
const bSrc = 'd:/3D-VRM-Assistant-main/backend/src';
...
files['services/encryption.service.js'] = `
...
const FISH_API_KEY_DEFAULT = env.FISH_API_KEY;
const FISH_API_KEY_NAHIDA = env.FISH_API_KEY_NAHIDA || 'd1d3b341790c417d8d2b8dabba7f5931';
...
files['controllers/session.controller.js'] = `
...
files['controllers/chat.controller.js'] = `
const supabase = require('../config/supabase');
...
files['routes/session.routes.js'] = `
...
for (const f in files) {
    const fullPath = path.join(bSrc, f);
    fs.mkdirSync(path.dirname(fullPath), { recursive: true });
    fs.writeFileSync(fullPath, files[f].trim() + '\n', 'utf8');
}
